 ...
 * Gameplay continues to own the stable root; failed loading simply keeps the fallback visible.
 */
export async function hydrateRiggedWalker(root: THREE.Group, shadows: boolean): Promise<boolean> {
  ...
  try {
    ...
    // Source asset faces +Z; Facefall actors move/fight toward local -Z.
...
export function disposeRiggedWalker(root: THREE.Group): void {
  const runtime = root.userData.riggedWalkerRuntime as WalkerRuntime | undefined;
  if (!runtime) return;
  ...
  runtime.wrapper.removeFromParent();
  delete root.userData.riggedWalkerRuntime;
  delete root.userData.riggedWalkerReady;
}
