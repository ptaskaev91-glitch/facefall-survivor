...
/**
 * Owns the render/world lifecycle that used to live directly in GameApp.
 * Gameplay systems may query collision/scene/camera, but they do not construct
 * renderer, fallback geometry, environment effects or manifest lights.
 */
export class WorldRuntime {
  ...
  constructor(
    ...
    events: EventBus<FacefallEvents>
  ) {
  ...
  updateCamera(playerPosition: THREE.Vector3, facing: THREE.Vector3, dt: number): void {
    this.cameraRig.update(playerPosition, facing, dt);
  }

  updateEnvironment(dt: number, playerPosition: THREE.Vector3): void {
